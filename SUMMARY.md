# Table of contents

* [👋 欢迎](README.md)

## 文档 <a href="#docs" id="docs"></a>

* [⚡ 快速开始](docs/getting-started/README.md)
  * [IDE 支持](docs/getting-started/ide-support/README.md)
    * [JetBrains Rider 设置](docs/getting-started/ide-support/jetbrains-rider-setup.md)
  * [用 Avalonia 开发](docs/getting-started/programming-with-avalonia/README.md)
    * [Model-View-ViewModel (MVVM)部分](docs/getting-started/programming-with-avalonia/the-model-view-viewmodel-pattern-mvvm.md)
    * [控制和布局](docs/getting-started/programming-with-avalonia/controls-and-layouts.md)
    * [数据绑定](docs/getting-started/programming-with-avalonia/data-binding.md)
    * [图像和动画](docs/getting-started/programming-with-avalonia/graphics-and-animations.md)
  * [Windows](docs/getting-started/windows.md)
  * [用户控件](docs/getting-started/usercontrols.md)
  * [资源](docs/getting-started/assets.md)
  * [开发工具](docs/getting-started/developer-tools.md)
  * [错误和警告日志](docs/getting-started/logging-errors-and-warnings.md)
  * [未处理的异常](docs/getting-started/unhandledexceptions.md)
  * [应用生命周期](docs/getting-started/application-lifetimes.md)
* [🔁 数据绑定](docs/data-binding/README.md)
  * [数据上下文](docs/data-binding/the-datacontext.md)
  * [变化通知](docs/data-binding/change-notifications.md)
  * [绑定](docs/data-binding/bindings.md)
  * [编译绑定](docs/data-binding/compiledbindings.md)
  * [与控件绑定](docs/data-binding/binding-to-controls.md)
  * [转换绑定值](docs/data-binding/converting-binding-values.md)
  * [Binding to Commands](docs/data-binding/binding-to-commands.md)
  * [Binding to Tasks and Observables](docs/data-binding/binding-to-tasks-and-observables.md)
  * [Binding from Code](docs/data-binding/binding-from-code.md)
  * [Binding in a Control Template](docs/data-binding/binding-in-a-control-template.md)
  * [Binding Classes](docs/data-binding/binding-classes.md)
  * [Creating and binding Attached Properties](docs/data-binding/creating-and-binding-attached-properties.md)
* [🎨 样式](docs/styling/README.md)
  * [样式](docs/styling/styles.md)
  * [选择器](docs/styling/selectors.md)
  * [资源](docs/styling/resources.md)
  * [疑难解答](docs/styling/troubleshooting.md)
* [🧰 控件](docs/controls/README.md)
  * [AutoCompleteBox](docs/controls/autocompletebox.md)
  * [Border](docs/controls/border.md)
  * [Buttons](docs/controls/buttons/README.md)
    * [Button](docs/controls/buttons/button.md)
    * [RepeatButton](docs/controls/buttons/repeatbutton.md)
    * [RadioButton](docs/controls/buttons/radiobutton.md)
    * [ToggleButton](docs/controls/buttons/togglebutton.md)
    * [ButtonSpinner](docs/controls/buttons/buttonspinner.md)
    * [SplitButton](docs/controls/buttons/splitbutton.md)
    * [ToggleSplitButton](docs/controls/buttons/togglesplitbutton.md)
  * [Calendar](docs/controls/calendar.md)
  * [Canvas](docs/controls/canvas.md)
  * [Carousel](docs/controls/carousel.md)
  * [CheckBox](docs/controls/checkbox.md)
  * [ComboBox](docs/controls/combobox.md)
  * [ContentControl](docs/controls/contentcontrol.md)
  * [ContextMenu](docs/controls/contextmenu.md)
  * [Decorator](docs/controls/decorator.md)
  * [DataGrid](docs/controls/datagrid/README.md)
    * [DataGridColumns](docs/controls/datagrid/datagridcolumns.md)
  * [DatePicker](docs/controls/datepicker.md)
  * [DockPanel](docs/controls/dockpanel.md)
  * [Expander](docs/controls/expander.md)
  * [Flyouts](docs/controls/flyouts.md)
  * [Grid](docs/controls/grid.md)
  * [GridSplitter](docs/controls/gridsplitter.md)
  * [Image](docs/controls/image.md)
  * [ItemsControl](docs/controls/itemscontrol.md)
  * [ItemsRepeater](docs/controls/itemsrepeater.md)
  * [LayoutTransformControl](docs/controls/layouttransformcontrol.md)
  * [ListBox](docs/controls/listbox.md)
  * [MaskedTextBox](docs/controls/maskedtextbox.md)
  * [Menu](docs/controls/menu.md)
  * [NativeMenu](docs/controls/nativemenu.md)
  * [NumericUpDown](docs/controls/numericupdown.md)
  * [Panel](docs/controls/panel.md)
  * [ProgressBar](docs/controls/progressbar.md)
  * [RelativePanel](docs/controls/relativepanel.md)
  * [ScrollBar](docs/controls/scrollbar.md)
  * [ScrollViewer](docs/controls/scrollviewer.md)
  * [Separator](docs/controls/separator.md)
  * [Slider](docs/controls/slider.md)
  * [SplitView](docs/controls/splitview.md)
  * [StackPanel](docs/controls/stackpanel.md)
  * [TabControl](docs/controls/tabcontrol.md)
  * [TabStrip](docs/controls/tabstrip.md)
  * [TextBlock](docs/controls/textblock.md)
  * [TrayIcon](docs/controls/trayicon.md)
  * [TreeDataGrid](docs/controls/treedatagrid/README.md)
    * [Creating a Hierarchical TreeDataGrid](docs/controls/treedatagrid/creating-a-hierarchical-treedatagrid.md)
    * [Creating a Flat TreeDataGrid](docs/controls/treedatagrid/creating-a-flat-treedatagrid.md)
    * [TreeDataGrid column types](docs/controls/treedatagrid/treedatagrid-column-types.md)
  * [TimePicker](docs/controls/timepicker.md)
  * [TextBox](docs/controls/textbox.md)
  * [ToolTip](docs/controls/tooltip.md)
  * [TreeView](docs/controls/treeview.md)
  * [TransitioningContentControl](docs/controls/transitioningcontentcontrol.md)
  * [UserControl](docs/controls/usercontrol.md)
  * [Viewbox](docs/controls/viewbox.md)
  * [Window](docs/controls/window.md)
  * [WrapPanel](docs/controls/wrappanel.md)
* [📚 模板](docs/templates/README.md)
  * [数据模板](docs/templates/data-templates.md)
  * [在代码中创建数据模板](docs/templates/creating-data-templates-in-code.md)
  * [实现 IDataTemplate 接口](docs/templates/implement-idatatemplate.md)
* [✏ 自定义控件](docs/authoring-controls/README.md)
  * [控件类别](docs/authoring-controls/types-of-control.md)
  * [定义属性](docs/authoring-controls/defining-properties.md)
* [🖱 输入](docs/input/README.md)
  * [路由事件](docs/input/routed-events.md)
  * [剪贴板](docs/input/clipboard.md)
  * [Mouse\&Pointer devices](docs/input/pointer.md)
  * [快捷键](docs/input/hotkeys.md)
* [🔑 动画](docs/animations/README.md)
  * [关键帧动画](docs/animations/keyframe-animations.md)
  * [过渡](docs/animations/transitions.md)
  * [页面过渡](docs/animations/pagetransitions.md)
* [📐 布局](docs/layout/README.md)
  * [面板概述](docs/layout/panels-overview.md)
  * [Alignment、Margin 和 Padding](docs/layout/alignment-margins-and-padding.md)
  * [创建自定义面板](docs/layout/create-a-custom-panel.md)
* [📦 发布/分发](docs/distribution-publishing/README.md)
  * [macOS](docs/distribution-publishing/macos.md)

## API 参考 <a href="#api" id="api"></a>

* [🗒 命名空间](api/namespace/README.md)
  * [Avalonia](api/namespace/avalonia.md)
  * [Avalonia.Animation](api/namespace/avalonia.animation/README.md)
    * [Avalonia.Animation.Easings](api/namespace/avalonia.animation/avalonia.animation.easings.md)
    * [Avalonia.Animation.Animators](api/namespace/avalonia.animation/avalonia.animation.animators.md)
  * [Avalonia.Collections](api/namespace/avalonia.collections.md)
  * [Avalonia.Controls](api/namespace/avalonia.controls.md)
  * [Avalonia.Data](api/namespace/avalonia.data/README.md)
    * [Avalonia.Data.Core.Plugins](api/namespace/avalonia.data/avalonia.data.core.plugins.md)
    * [Avalonia.Data.Core](api/namespace/avalonia.data/avalonia.data.core.md)
    * [Avalonia.Data.Converters](api/namespace/avalonia.data/avalonia.data.converters.md)
  * [Avalonia.Diagnostics](api/namespace/avalonia.diagnostics.md)
  * [Avalonia.Dialogs](api/namespace/avalonia.dialogs.md)

## 指南 <a href="#guides" id="guides"></a>

* [🐣 基础](guides/basics/README.md)
  * [Introduction to XAML](guides/basics/introduction-to-xaml.md)
  * [Code-behind](guides/basics/code-behind.md)
  * [MVVM Architecture](guides/basics/mvvm-architecture.md)
  * [Accessing the UI thread](guides/basics/accessing-the-ui-thread.md)
* [🤿 深入](guides/deep-dives/README.md)
  * [Running your app on a Raspberry Pi](guides/deep-dives/running-your-app-on-a-raspberry-pi.md)
  * [Running your app on a Raspberry Pi with Raspbian Lite](guides/deep-dives/running-your-app-on-a-raspberry-pi-with-raspbian-lite.md)
  * [ReactiveUI](guides/deep-dives/reactiveui/README.md)
    * [View Activation](guides/deep-dives/reactiveui/view-activation.md)
    * [Routing](guides/deep-dives/reactiveui/routing.md)
    * [Data Persistence](guides/deep-dives/reactiveui/data-persistence.md)
    * [Binding to Sorted/Filtered Data](guides/deep-dives/reactiveui/binding-to-sorted-filtered-data.md)
* [👩💻 👩💻 开发人员指南](guides/developer-guides/README.md)
  * [🏭 Build Avalonia from Source](guides/developer-guides/build-avalonia-from-source.md)
  * [Comparison of Avalonia with WPF and UWP](guides/developer-guides/comparison-of-avalonia-with-wpf-and-uwp.md)
  * [Debugging Previewer](guides/developer-guides/debugging-previewer.md)
  * [Debugging the XAML compiler](guides/developer-guides/debugging-the-xaml-compiler.md)
  * [macOS Development](guides/developer-guides/macos-development.md)
  * [Release Process](guides/developer-guides/release-process.md)
  * [Maintaining Stable Branch](guides/developer-guides/maintaining-stable-branch.md)

## 教程 <a href="#tutorials" id="tutorials"></a>

* [📋 待办事项应用](tutorials/todo-list-app.md)
* [📻 音乐商店应用](tutorials/music-store-app.md)
* [🌐 在浏览器中运行](tutorials/running-in-the-browser.md)
* [📱 为移动设备开发](tutorials/developing-for-mobile.md)

## 杂项 <a href="#misc" id="misc"></a>

* [👪 社区](misc/community.md)
* [🖥 WPF 开发者建议](misc/wpf.md)
* [📋 正在使用 Avalonia 的项目](misc/projects-that-are-using-avalonia.md)
* [❔ 常见问题](misc/faq.md)
